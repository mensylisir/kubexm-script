...
step::kubeadm.init.master::run() {
  ...
  kubeadm::generate_init_config "${config_file}" "${first_master}" || { log::error "Failed to generate kubeadm init config"; return 1; }
  ...
  if [[ -f "${packages_dir}/audit-policy.yaml" ]]; then
    runner::remote_copy_file "${packages_dir}/audit-policy.yaml" "/etc/kubernetes/audit-policy.yaml" || { log::error "Failed to copy audit policy to ${KUBEXM_HOST}"; return 1; }
  fi
  if [[ -f "${packages_dir}/encryption-config.yaml" ]]; then
    runner::remote_copy_file "${packages_dir}/encryption-config.yaml" "/etc/kubernetes/encryption-config.yaml" || { log::error "Failed to copy encryption config to ${KUBEXM_HOST}"; return 1; }
  ...
  if [[ ${wait_count} -ge 30 ]]; then
    log::error "Control plane components did not start within 60 seconds"
    return 1
  fi

  log::info "kubeadm init completed on ${KUBEXM_HOST}"
}

step::kubeadm.init.master::rollback() { return 0; }

step::kubeadm.init.master::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
