...
step::kubeadm.join.worker.collect.endpoint::check() { return 1; }

step::kubeadm.join.worker.collect.endpoint::run() {
  local ctx="$1"
  shift
  ...
  first_master_ip="$(context::get "kubeadm_join_worker_first_master_ip" || true)"
  ...
  context::set "kubeadm_join_worker_apiserver_endpoint" "${apiserver_endpoint}"
  context::set "kubeadm_join_worker_cri_socket" "${cri_socket}"
}

step::kubeadm.join.worker.collect.endpoint::rollback() { return 0; }

step::kubeadm.join.worker.collect.endpoint::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
