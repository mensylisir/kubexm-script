...
step::lb.external.kubexm.kh.collect.keepalived.settings::check() { return 1; }

step::lb.external.kubexm.kh.collect.keepalived.settings::run() {
  local ctx="$1"
  shift
  ...
  context::set "lb_kh_keepalived_vip" "${vip}"
  context::set "lb_kh_keepalived_interface" "${interface}"
  context::set "lb_kh_keepalived_state" "${state}"
  context::set "lb_kh_keepalived_priority" "${priority}"
  context::set "lb_kh_keepalived_node_index" "${node_index}"
  context::set "lb_kh_keepalived_unicast_peers" "$(echo -e "${unicast_peers}")"
  context::set "lb_kh_keepalived_router_id" "${router_id}"
  context::set "lb_kh_keepalived_auth_pass" "${auth_pass}"
}

step::lb.external.kubexm.kh.collect.keepalived.settings::rollback() { return 0; }

step::lb.external.kubexm.kh.collect.keepalived.settings::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
