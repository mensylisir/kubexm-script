...
step::lb.external.kubexm.kn.collect.keepalived.identity::check() { return 1; }

step::lb.external.kubexm.kn.collect.keepalived.identity::run() {
  local ctx="$1"
  shift
  ...
  context::set "lb_kn_keepalived_dir" "${lb_dir}"
}

step::lb.external.kubexm.kn.collect.keepalived.identity::rollback() { return 0; }

step::lb.external.kubexm.kn.collect.keepalived.identity::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
