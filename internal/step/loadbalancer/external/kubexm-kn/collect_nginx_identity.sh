...
step::lb.external.kubexm.kn.collect.nginx.identity::check() { return 1; }

step::lb.external.kubexm.kn.collect.nginx.identity::run() {
  local ctx="$1"
  shift
  ...
  context::set "lb_kn_nginx_dir" "${lb_dir}"
}

step::lb.external.kubexm.kn.collect.nginx.identity::rollback() { return 0; }

step::lb.external.kubexm.kn.collect.nginx.identity::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
