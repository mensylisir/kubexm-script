...
step::lb.external.kubexm.kn.copy.nginx.config::check() {
  source "${KUBEXM_ROOT}/internal/step/common/checks.sh"
  step::check::remote_file_exists "${KUBEXM_HOST}" "/etc/nginx/nginx.conf"
}

step::lb.external.kubexm.kn.copy.nginx.config::run() {
  local ctx="$1"
  shift
  ...
  local cfg_file
  cfg_file="${KUBEXM_ROOT}/packages/${cluster_name}/${node_name}/loadbalancer/kubexm-kn/nginx.conf"
  if [[ ! -f "${cfg_file}" ]]; then
...
step::lb.external.kubexm.kn.copy.nginx.config::rollback() { return 0; }

step::lb.external.kubexm.kn.copy.nginx.config::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
