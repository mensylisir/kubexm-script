...
step::etcd.render.config::check() {
  ...
  if [[ -f "${etcd_dir}/etcd.config.yml" ]]; then
    return 0
  fi
  return 1
}

step::etcd.render.config::run() {
  local ctx="$1"
  shift
  local cluster_name="${KUBEXM_CLUSTER_NAME:-}"
  local arg
  ...
  etcd::generate_config "${node_name}" "${node_ip}" "${etcd_dir}"
}

step::etcd.render.config::rollback() { return 0; }

step::etcd.render.config::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
