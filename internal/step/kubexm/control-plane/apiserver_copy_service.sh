...
step::kubernetes.apiserver.copy.service::run() {
  ...
  # 部署 audit policy 和 encryption config（生产必需）
  local packages_base="${KUBEXM_ROOT}/packages/${cluster_name}"
  runner::remote_exec "mkdir -p /etc/kubernetes /var/log/kubernetes" || true
  ...
  if [[ -f "${packages_base}/audit-policy.yaml" ]]; then
    ...
    log::warn "audit-policy.yaml not found, apiserver may fail to start if audit enabled"
  fi
  ...
  if [[ -f "${packages_base}/encryption-config.yaml" ]]; then
    ...
    log::warn "encryption-config.yaml not found, etcd secrets will be stored unencrypted"
  fi
}
