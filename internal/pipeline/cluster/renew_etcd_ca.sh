...
pipeline::renew_etcd_ca() {
  local ctx="$1"
  shift
  KUBEXM_PIPELINE_NAME="renew.etcd-ca"
  if [[ "${KUBEXM_DRY_RUN:-false}" == "true" ]]; then
    logger::info "DRY-RUN enabled: planning renew etcd-ca pipeline"
    return 0
  fi
  ...
  if [[ -z "${cluster_name}" ]]; then
    logger::error "missing required --cluster for renew etcd-ca"
    return 2
  fi
  ...
  logger::info "[Pipeline] Starting ETCD CA renewal..."
  module::certs_renew_etcd_ca "${ctx}" "$@" || { pipeline::step_fail "CertRenew"; return $?; }
  ...
  trap - EXIT
  pipeline::summary
  logger::info "[Pipeline:renew_etcd_ca] ETCD CA renewed successfully!"
  return 0
}