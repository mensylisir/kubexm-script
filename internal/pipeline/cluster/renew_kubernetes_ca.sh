...
pipeline::renew_kubernetes_ca() {
  local ctx="$1"
  shift
  KUBEXM_PIPELINE_NAME="renew.kubernetes-ca"
  if [[ "${KUBEXM_DRY_RUN:-false}" == "true" ]]; then
    logger::info "DRY-RUN enabled: planning renew kubernetes-ca pipeline"
    return 0
  fi
  ...
  if [[ -z "${cluster_name}" ]]; then
    logger::error "missing required --cluster for renew kubernetes-ca"
    return 2
  fi
  ...
  logger::info "[Pipeline] Starting Kubernetes CA renewal..."
  module::certs_renew_kubernetes_ca "${ctx}" "$@" || { pipeline::step_fail "CertRenew"; return $?; }
  ...
  trap - EXIT
  pipeline::summary
  logger::info "[Pipeline:renew_kubernetes_ca] Kubernetes CA renewed successfully!"
  return 0
}