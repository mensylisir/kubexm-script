...
step::cluster.scale.kubeadm.reset::check() {
...
step::cluster.scale.kubeadm.reset::run() {
  local ctx="$1"
  shift
...
step::cluster.scale.kubeadm.reset::rollback() { return 0; }

step::cluster.scale.kubeadm.reset::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
