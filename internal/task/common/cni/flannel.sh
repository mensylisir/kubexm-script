...
step::cluster.install.cni.flannel::check() {
...
step::cluster.install.cni.flannel::run() {
  local ctx="$1"
  shift
  ...
  if [[ -z "${first_master}" ]]; then
    log::error "No control-plane nodes found for Flannel install"
    return 1
  fi

  local k8s_version flannel_version manifest_file
  k8s_version=$(config::get_kubernetes_version)
  flannel_version=$(versions::get "flannel" "${k8s_version}" || defaults::get_flannel_version)
  manifest_file="${KUBEXM_ROOT}/packages/${cluster_name}/${first_master}/flannel/${flannel_version}/kube-flannel.yml"
  if [[ ! -f "${manifest_file}" ]]; then
    log::error "Flannel manifest not found: ${manifest_file}"
    return 1
  fi

  if kubectl --kubeconfig="${kubeconfig}" apply -f "${manifest_file}" >/dev/null 2>&1; then
    log::success "Flannel CNI plugin applied"
  else
    log::error "Failed to apply Flannel CNI manifest"
    return 1
  fi
}

step::cluster.install.cni.flannel::rollback() { return 0; }

step::cluster.install.cni.flannel::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
