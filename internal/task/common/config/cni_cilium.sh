...
step::cluster.config.dirs.cni.cilium::check() {
  ...
  cni_version=$(versions::get "cilium" "${k8s_version}" || defaults::get_cilium_version)
  if [[ -d "${cluster_dir}/${first_master}/cilium/${cni_version}" ]]; then
    return 0  # dir exists, skip
  fi
  return 1  # need to create
}

step::cluster.config.dirs.cni.cilium::run() {
  local ctx="$1"
  shift
  ...
  local cni_version
  cni_version=$(versions::get "cilium" "${k8s_version}" || defaults::get_cilium_version)
  mkdir -p "${cluster_dir}/${first_master}/cilium/${cni_version}"
}

step::cluster.config.dirs.cni.cilium::rollback() { return 0; }

step::cluster.config.dirs.cni.cilium::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
