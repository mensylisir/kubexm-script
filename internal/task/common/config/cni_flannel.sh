...
step::cluster.config.dirs.cni.flannel::check() {
  ...
  cni_version=$(versions::get "flannel" "${k8s_version}" || defaults::get_flannel_version)
  if [[ -d "${cluster_dir}/${first_master}/flannel/${cni_version}" ]]; then
    return 0  # dir exists, skip
  fi
  return 1  # need to create
}

step::cluster.config.dirs.cni.flannel::run() {
  local ctx="$1"
  shift
  ...
  local cni_version
  cni_version=$(versions::get "flannel" "${k8s_version}" || defaults::get_flannel_version)
  mkdir -p "${cluster_dir}/${first_master}/flannel/${cni_version}"
}

step::cluster.config.dirs.cni.flannel::rollback() { return 0; }

step::cluster.config.dirs.cni.flannel::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
