...
step::cluster.scale.stop.kubelet::check() {
...
step::cluster.scale.stop.kubelet::run() {
  local ctx="$1"
  shift
...
step::cluster.scale.stop.kubelet::rollback() { return 0; }

step::cluster.scale.stop.kubelet::targets() {
  : "${KUBEXM_ROOT:?KUBEXM_ROOT is required}"
